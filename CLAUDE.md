...
1. ~~**Migrate Vol I pages to live data.**~~ **Done 2026-05-01:** Five-pillar-scorer, watch-opportunity-ledger, and company.html now load `data/prices.json` via `data-loader.js` and merge live volatile fields (pe, peg, gm, fcfY, fwdG for Core; ev, evRev, growth, evRevGrowth for Watch) into the embedded snapshot before rendering. Curated fields (moat, vintage, regulatory, ROIC/WACC/spread, TAM, notes, thesis, bull/bear) stay from the embedded data — they aren't on yfinance. Each page shows a "Live data · refreshed [date]" badge that falls back to "Embedded snapshot" if the fetch fails. **Still open:** consolidate inline ledger data and `data/core.json`/`data/watch.json` into a single source of truth (item 6 below).
...
5. **Backtester (partial — 2026-05-01).** `backtest.html` is built and `data/returns.json` is populated weekly via the GitHub Action (Sundays at 22:00 UTC, or on-demand via `workflow_dispatch` with `with_returns: true`). The page joins current composite scores to historical 1y/3y/5y total returns from yfinance, shows average return by verdict band, a composite-vs-5y-return scatter with linear fit and Pearson correlation, and a per-company table with SPY benchmark spread. **Honest about limitations:** the curated inputs (ROIC, moat, vintage, regulatory) are frozen at the most recent quarterly review and cannot be recomputed historically — so this answers "did names that score well today have compounded historically" rather than the stricter "did the framework predict future winners." Survivorship bias applies. **Still open:** rolling time-series of composite (would need historical fundamentals), forward-looking validation (compute scores today, track returns going forward).
